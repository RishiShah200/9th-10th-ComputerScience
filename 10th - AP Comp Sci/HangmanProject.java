...
reader.nextLine();
		String[] lettersGuessed = new String [25];		//Huge array so that the user can guess many letters without out of bounds error
		String[] output = new String [finalWord.length()+1];	//Array for the final word output with dashes (r_T-_ _)
		String[] wordFinal = new String [finalWord.length()+1];		//Stores the final word in an array
...
System.out.println(finalWord);
		while(guesses!=0){		//While loop condition that only runs when the number of misses the user has is not equals to zero
			...
			for(int i = 0;i<finalWord.length();i++){		//Runs to check if they correctly guessed the word and outputs that they won if they did
		...
		public static String wordChoice3(int a){
				String[] answers = {"basket","backpack","sriracha","xanthosis","computers","fortnite","sleeping","football","keyboard","jalapenos"};
				String realWord = answers[a];
				return realWord;
		}
